...
test("ships the routing, evaluation, fallback and export surfaces", async () => {
  ...
  assert.doesNotMatch(router, /EXPERIMENT_SUMMARIES|buildExperimentCsv|EVALUATION_QUERIES/);
  assert.match(source, /api\/evaluations\?datasetVersion=2\.0\.0/);
  assert.match(source, /实验状态与证据说明/);
  assert.match(source, /同一批任务，比较四种策略。/);
  assert.match(source, /全 Mini/);
  assert.match(source, /全 Lite/);
  assert.match(source, /全 Pro/);
  assert.match(source, /动态路由/);
  assert.match(source, /问题案例与纠正记录/);
  assert.match(source, /评测驱动，让策略持续校准。/);
  assert.match(source, /样本不足，暂不下结论/);
  assert.match(source, /尚无已运行案例/);
  assert.doesNotMatch(source, /导出 300 条原始结果/);
  assert.doesNotMatch(source, /100 条合成任务/);
  assert.doesNotMatch(source, /3 个策略组/);
  assert.doesNotMatch(source, /原 AgentScope 智能体评测/);
  assert.doesNotMatch(source, /失败不是脏数据/);
  assert.doesNotMatch(source, /供应商可替换/);
  assert.doesNotMatch(source, /模型不写死，门槛可解释。/);
  assert.match(source, /硬约束、可执行性与安全要求/);
  assert.match(source, /只有全部预期试次完成后才计算指标/);
